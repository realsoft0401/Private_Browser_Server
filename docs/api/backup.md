...
- 负责创建中心 task、同步调用 Edge backup、刷新中心环境摘要
...
接口会先创建中心 task，再同步调用 Edge `/backup` 并立即完成中心收口。
因此 HTTP 200 返回时，当前 backup 已经完成成功，不需要再等待 Edge task：
...
- `taskId` 是中心任务 ID
- 当前实现不会创建 `edgeTaskId`，因为 Edge backup 是同步接口
...
POST /api/v1/envs/{envId}/backup
  ...
  -> 校验 Edge 同步返回 status=backed_up
  -> 回写 server_tasks=success 与 server_browser_envs.status=backed_up
...
1. Edge `/api/v1/edge/browser-envs/{envId}/backup` 同步返回成功
2. 返回体里的 `status=backed_up`
...
```text
Edge backup result -> status == backed_up
```
...
- 创建中心 task 失败
- 调 Edge `/backup` 失败
- Edge `/backup` 返回成功但 `status` 不是 `backed_up`
...
- `envId`
- `edgeTaskId`
  - 当前 backup 正常情况下为空，因为 Edge backup 不返回 taskId；该字段保留给统一日志结构复用
- `stage`
- `status`
- `errorSource`
- `message`
...
- `Edge backup 返回成功，但状态不是 backed_up；Node Server 无法确认资产已完成归档，请管理员检查 Edge 服务日志与环境包详情`
...
- Node Server `POST /api/v1/envs/{envId}/backup` 第一版已落地
- 当前实现采用“中心 task + 同步 Edge backup + 立即中心收口”模式，不绑定 `edgeTaskId`
- 本文档继续作为后续联调验收和 `restore/revalidate/import-package` 对齐的参考样板
