...
## 3.11 解绑后下次 bind 必须重新生成 `clientId`

同一台物理 Client：

- 解绑
- 清空 Client 本地 `node-registration.json`
- 后续再次 bind
...
- 你已经把 unbind 的边界收紧为“删除当前有效绑定结果”
- 解绑后就不应再保留一个可被直接复用的有效中心身份
- 下次接入必须重新完成 discovery -> bind -> 写回的全流程

同时要明确：

- 历史审计仍保留
- 历史 bind / unbind 留痕仍保留
- 删除的是当前有效绑定，不是删除审计记录
